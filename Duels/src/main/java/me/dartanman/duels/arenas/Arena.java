...
/**
 * Arena - Contains information for a duel and several methods pertaining to a
 * duel
 * 
...
public class Arena
{
    ...
    {
	String winMessage = plugin.getConfig().getString("Messages.WinMessage");
	winner.sendMessage(StringUtils.color(winMessage));
	plugin.getStatUtils().addWin(winner);
	PlayerRestorationInfo pri = new PlayerRestorationInfo(null);
	try
	{
	    String loseMessage = plugin.getConfig().getString("Messages.LoseMessage");
	    loser.sendMessage(StringUtils.color(loseMessage));
	}
	catch (NullPointerException e)
	{
	    // loser disconnected, they didn't die. As such, they are not here to be sent a
	    // message.
	}
	...
	if (spawn1 == null || spawn2 == null)
	{
		...
		p.sendMessage(ChatColor.RED
			+ "Unfortunately, this Arena has been set up incorrectly. Contact an Administrator to fix this!");
	...
	for (Player p : players)
	{
	    ...
	    String prepareMessage = plugin.getConfig().getString("Messages.PrepareToDuelMessage");
	    p.sendMessage(StringUtils.color(prepareMessage));
     ...
     * 
     * @return Whether or not the duel can start
     */
     ...
     * 
     * @return Active status of the arena
     */
